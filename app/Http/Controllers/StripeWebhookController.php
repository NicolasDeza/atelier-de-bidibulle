...
class StripeWebhookController extends Controller
{
    public function handle(Request $request)
    {
        ...
        Log::info('[Stripe] Event reçu', ['type' => $event->type]);

        if ($event->type === 'checkout.session.completed') {
            $this->handleCheckoutCompleted($event->data->object);
        } elseif ($event->type === 'payment_intent.succeeded') {
            Log::info('[Stripe] payment_intent.succeeded', ['pi' => $event->data->object->id]);
            ...
            Log::info('[Stripe] Événement ignoré', ['type' => $event->type]);
        }
    ...
    {
        // Recharger la session avec expand (mais PAS shipping_details)
        ...
        try {
            ...
            Log::info('[Stripe] Session rechargée', [
        ...
        // Fallback sur PaymentIntent (montants + adresse via charge)
        $pi = null;
        try {
        ...
        try {
            DB::transaction(function () use ($session, $pi) {
                ...
                if ($shippingLabel !== null) {
                    $order->shipping_method_label = $shippingLabel; // ✅ Mode livraison
                }
                $order->shipping_total = $shippingEuro; // ✅ Frais livraison
                if ($address) {
                    $order->shipping_address_json = json_encode($address, JSON_UNESCAPED_UNICODE); // ✅ Adresse
                }
                if ($amountTotalCts > 0) {
                ...
                Log::info('[Stripe] Order mis à jour', [
                    'order_id' => $order->id,
                    'uuid'     => $order->uuid,
                    'label'    => $shippingLabel,
                    'ship€'    => $shippingEuro,
